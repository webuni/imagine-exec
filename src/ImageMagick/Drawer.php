...
final class Drawer implements DrawerInterface
{
    ...
    public function ellipse(PointInterface $center, BoxInterface $size, ColorInterface $color, $fill = false, $thickness = 1)
    {
        ...
        try {
            ...
            $arguments[] = '-stroke '.$pixel->getColorAsString();
            $arguments[] = '-strokewidth '.max(1, (int) $thickness);

            if ($fill) {
                $arguments[] = '-fill '.$pixel->getColorAsString();
            } else {
                $arguments[] = '-stroke transparent';
            }

            $arguments[] = sprintf('-draw "ellipse %d,%d %d,%d 0,360"', $center->getX(), $center->getY(), $width/2, $height/2);
